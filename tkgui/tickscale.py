...
class TickScale(ttk.Frame):
    ...
    def __init__(self, master=None, **kwargs):
        ...
        if self._resolution > 0 and False:
        ...
        if self._tickinterval == 0 and self._resolution == 0:
            ...
            if 0 <= self._digits < d:
                ...
                if self._resolution > self._tickinterval: self._tickinterval = self._resolution
                #self.scale.configure(from_=round(self.scale.get('from'), self._digits), to=round(self.scale.get('to'), self._digits))
    ...
    def _init(self):
        ...
        for label in self.ticklabels: label.destroy()
        ...
        if self._resolution > 0:
            ...
            #self.scale.configure(to=self.scale.cget('from') + nb_steps * self._resolution)
            #self._extent = self.scale.cget('to') - self.scale.cget('from')
        print("T",self._start, self._extent, self._tickinterval)
